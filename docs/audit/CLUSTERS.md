...
| 5 | Admissions | `drift` | www has institutional admissions content at `/resources/future-students/admissions-process-requirements-and-criteria` (Stage 2 patch caught the Stage 1 miss) + 9 URLs per campus | `admissions.html` | 2026-05-03 | **Drift detected 2026-05-03 on 2 URLs (1 page, both campus mirrors): `acceptable-proofs-of-residency` now inlines the full text of Florida Statute 1009.21 (1328 → 15319 chars). Other size deltas in the same drift run are cosmetic (markdown vs innerText extraction overhead) — see `admissions/DRIFT-LOG.md`. Reconciliation pending Marianne approval; redesign untouched.** |
