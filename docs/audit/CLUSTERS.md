...
| 6 | Tuition | `extracting` | www has placeholder only at `/resources/future-students/financial-aid` (200 OK, empty body). Real content lives on campus subsites: CLW + STP each publish a 9-URL `/admissions/financial-aid/...` sub-tree (hub + 8 sub-pages). One asymmetry: `/admissions/military-veteran-student-resources` exists CLW only. NPC slug is campus-specific by IPEDS design. | `tuition-aid.html` | 2026-05-04 | **Stage 1 inventory complete 2026-05-04.** 19 URLs queued for Stage 2 (10 CLW including military-vet asymmetric, 9 STP, 0 www extractable). See `tuition/inventory.md`. Stage 2 uses WebFetch rendered DOM per the open pipeline-infrastructure follow-up (curl+bs4 JS-render gap caught 2026-05-03). |
