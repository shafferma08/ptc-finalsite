...
| 3 | Compliance | `extracting` | www PCS/CTAE Compliance Statements + Title IX language + ESE info + accreditation info per campus + 8 unsourced sections on `consumer-information.html` deferred from about-sub-pages (FERPA, Accessibility/504/ADA, Student Outcomes, Drug & Alcohol, Copyright/P2P, Voter Registration, Constitution Day, ESE) | `consumer-information.html` is the redesign Compliance hub (IA decision: no separate /compliance page; CI hub already exists with 17 sections). | 2026-04-28 | **Stage 1 complete.** Research subagent surveyed live PTC + PCSB. Of 8 unsourced topics: 1 FOUND (Accessibility, split across `myptc.edu/accessibility-statement` + `myptc.edu/privacy-policy`), 1 DISTRICT-only (ESE), **6 NOT FOUND ANYWHERE** (FERPA, Student Outcomes, DFSCA, HEOA Copyright, Voter Reg, Constitution Day). Per verbatim rule those 6 sections get stripped from CI and logged as federal compliance gaps. Stage 2 next: extract content from the 2 found URLs + verify the named officers (Dena Collins EEO/Title IX/ADA + Stephanie Miller 504). About-cluster extracts cover the other CI sections. **Major scope shift: this cluster's main work is removal, not addition.** |
