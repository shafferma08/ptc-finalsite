...
| 5 | Admissions | `verified` | www has institutional admissions content at `/resources/future-students/admissions-process-requirements-and-criteria` (Stage 2 patch caught the Stage 1 miss) + 9 URLs per campus | `admissions.html`, `acceptable-proofs-of-residency.html` (NEW) | 2026-05-03 | **Drift reconciled 2026-05-03.** Marianne approved sub-page split for residency content (verbatim mirror of live with UX-aligned structure). Build: (1) re-extracted `acceptable-proofs-of-residency` for both campuses via WebFetch (rendered DOM) — saved 13.8K-char verbatim baseline that captures the JS-injected FL Statute 1009.21 body the original curl+bs4 extractor missed. (2) Built new shared sub-page `acceptable-proofs-of-residency.html` holding the full statute text verbatim (subsections 1-13 + History citation), with breadcrumb, TOC anchors, print-friendly styling, and back-link to admissions. (3) Updated `admissions.html#residency` to keep the verbatim lead paragraph (em-dash + bold restored to match live) and replaced the old 5-statute-link list with a single primary CTA pointing to the new sub-page. Stage 7 verifier: 6 of 7 blocks CONFIRM-RESOLVED, 1 FLIP fixed mid-run (em-dash + bold drift on lead paragraph), 0 NEW-DRIFT-INTRODUCED. New high-priority follow-up filed under "Pipeline infrastructure": curl+bs4 extractor missed JS-injected content (potentially affects prior verified clusters; needs backfill). 8 cosmetic-drift rows in DRIFT-LOG remain known noise; not real content changes. Drift-watched. |
