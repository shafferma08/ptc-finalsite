...
| 5 | Admissions | `verifying` | www has institutional admissions content at `/resources/future-students/admissions-process-requirements-and-criteria` (Stage 2 patch caught the Stage 1 miss) + 9 URLs per campus | `admissions.html` | 2026-04-30 | **Stage 6 build complete.** All 15 migration-order steps applied: hero rewritten from www framing; #how-to-apply rebuilt as 8-step ordered list verbatim from www admissions page + campus hubs (HS-diploma fabrication stripped, FAFSA School Code per-campus token added with both 005605/013917, all 8 steps source-cited via HTML comments); #enrollment-steps repurposed as "Who Can Apply and When" (age + start-dates verbatim); 2 NEW sections #enrollment-options (verbatim 2675 chars) + #residency (verbatim 1328 chars + 4 Florida-statute links); #pathways rewritten with full Transfer + Readmission verbatim (DE + Veterans cards stripped, replaced with one-line cross-cluster pointers); #testing rebuilt with verbatim CASAS overview (no "listening skills"), verbatim TEAS overview (no PCT/SST), per-campus CASAS + TEAS schedule cards including Merritt Scott contact verbatim with phone-format normalization; #campus-tours rebuilt with shadowing institutional framing + per-campus cards (CLW links April 2026 PDF, STP routes to counselor per D1); FAQ section stripped entirely (D2); #accommodations kept + cross-link to consumer-information#accessibility added (A10); CTA band re-aimed (Apply Now → apply.myptc.edu, Request Info → inforequest.myptc.edu); sticky in-page TOC added; 9 follow-ups appended to `docs/audit/follow-ups.md`. HTML structure validated (0 unclosed tags, 0 mismatches). Title em-dash swapped to pipe per Marianne's no-em-dashes rule. CSS expanded (~250 lines: .admissions-toc, .apply-steps, .campus-token, .campus-cards, .enrollment-options-grid, .statute-list, .testing-subhead). Ready for Stage 7 verifier dispatch. |
