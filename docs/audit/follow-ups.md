...
| Sitewide utility-bar "Apply Now" placeholder still `href="#"` | redesign chrome (all pages) | Verifier surfaced during Admissions Stage 7: the page-level Apply Now button was repointed to apply.myptc.edu, but the global utility-bar Apply Now in the header is still a placeholder. Affects every redesign page, not just admissions. | Sitewide chrome pass: repoint utility-bar Apply Now to https://apply.myptc.edu. Pair with the other utility-bar placeholder cleanup carried from the review panel (Student Portal, Events). | PTC webmaster (Marianne) | Low | Admissions cluster Stage 7, 2026-05-01 |
